# -*- coding: utf-8 -*-
"""
Problem definition for the full, multi-objective optimization case.
This version adds an additional inner-loop constraint for the hypersonic
design point to enforce a minimum center of pressure (Xcp) value.
"""
import torch
import pandas as pd
...
def define_problem() -> OptimizationConfig:
    """
    Defines and assembles the complete multi-objective optimization problem.
    """
    ...
    low_speed_pipeline = SurrogateBackedPipeline(
        mach_str="0.4",
        train_paths=low_speed_data,
        inner_loop_objective="CL"
    )
    
    high_speed_pipeline = SurrogateBackedPipeline(
        mach_str="10.0",
        train_paths=high_speed_data,
        inner_loop_objective="K"
    )
    ...
    design_points = [
            ...
            inner_opt_config=InnerOptConfig(
                ...
                constraints=[
                    InnerConstraint(
                        ...
                        eval_function=lambda res: res["K"] - 4.8
                    )
                ]
            )
        ),
        DesignPoint(
            ...
            inner_opt_config=InnerOptConfig(
                bounds=[(10.0, 25.0)], 
                x0=[15.0],
                # ** NEW: Add the Xcp constraint to the hypersonic inner loop **
                constraints=[
                    InnerConstraint(
                        name="CL_min_hypersonic",
                        description="CL @ Ma 10.0 >= 0.09",
                        eval_function=lambda res: res["CL"] - 0.085
                    ),
                    InnerConstraint(
                        name="Xcp_min_hypersonic",
                        description="Xcp @ Ma 10.0 >= 0.68",
    ...
    constraints = [
        GlobalConstraint(
            name="Xcp_Stability",
            description="|Xcp_sub - Xcp_hyper| (norm) <= 0.06",
            eval_function=lambda res_map: 0.06 - abs(res_map["Subsonic_Ma0.4"]["Xcp"] - res_map["Hypersonic_Ma10.0"]["Xcp"])
        )
    ]
    ...
    offline_bounds = get_offline_bounds_from_data(
        data_paths=low_speed_data + high_speed_data,
        offline_dim=15,
        device=DEVICE,
        dtype=DTYPE
    )
