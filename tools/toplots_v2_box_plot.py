...
def plot_figure_4_objective_space(df_30, df_38, output_path):
    ...
    for i, (ax, df) in enumerate(zip(axes, dfs)):
        ...
        if not feasible.empty:
            ...
            # Make the marker pop but avoid a heavy black border
            ax.scatter(
                best_feasible['obj_Maximize_Volume'],
                best_feasible['con_Maintain_L/D_Ratio'],
                marker='*',
                s=250,
                edgecolor='white',    # thin white edge to separate from mixed backdrops
                linewidths=0.9,
    ...
    fig.legend(handles, labels, loc='lower center', ncol=4, bbox_to_anchor=(0.5, 0.05),
               frameon=True, fancybox=True, shadow=True, fontsize=font_legend)
...
if __name__ == "__main__":
    ...
    plot_figure_5_boxplot_distribution(df_30, df_38, os.path.join(OUTPUT_DIR, 'Fig5_Final_Boxplot.png'))
