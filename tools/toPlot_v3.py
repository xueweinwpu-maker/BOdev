...
try:
    ...
    print(f"Error: Could not find data file. Details: {e}")
    exit()
...
def plot_figure_5_violin_distribution(df_30, df_38, output_path, n_top=20):
    """
    Generates a clear and elegant Figure 5 using violin plots to show the
    aggregated distribution of design variables by surface.
    """
    
    def prep_df_for_violin(df, n, windward_dvs_count, leeward_dvs_count, case_name):
        feasible = df[df['con_Maintain_L/D_Ratio'] >= 0].copy()
        top_feasible = feasible.nlargest(n, 'obj_Maximize_Volume')
        
        # Windward DVs are split in the 38-DV case
        if case_name == '38-DV':
            windward_cols = [f'offline_var_{i}' for i in range(12)] + [f'offline_var_{i}' for i in range(30, 38)]
        else:
             windward_cols = [f'offline_var_{i}' for i in range(windward_dvs_count)]
        
        leeward_cols = [f'offline_var_{i}' for i in range(windward_dvs_count, windward_dvs_count + leeward_dvs_count)]

        df_windward = top_feasible[windward_cols].melt(var_name='DV', value_name='Value')
        df_windward['Surface'] = 'Windward'
        
        df_leeward = top_feasible[leeward_cols].melt(var_name='DV', value_name='Value')
        df_leeward['Surface'] = 'Leeward'
        
        combined = pd.concat([df_windward, df_leeward], ignore_index=True)
        combined['Case'] = case_name
        return combined

    # Prepare data
    df_30_plot = prep_df_for_violin(df_30, n_top, 10, 20, '30-DV (WDA)')
    df_38_plot = prep_df_for_violin(df_38, n_top, 18, 20, '38-DV (FFD)')
    
    final_df = pd.concat([df_30_plot, df_38_plot], ignore_index=True)

    # Create the plot
    fig, ax = plt.subplots(figsize=(10, 6))
    
    sns.violinplot(ax=ax, data=final_df, x='Case', y='Value', hue='Surface',
                   split=True, inner='quart', palette={'Windward': colors['accent'], 'Leeward': colors['primary']},
                   linewidth=1.5)

    ax.axhline(0, color=colors['neutral'], linestyle='--', linewidth=1, alpha=0.8)
    ax.set_title('Aggregated Distribution of Top Design Variables by Surface')
    ax.set_xlabel('Optimization Case')
    ax.set_ylabel('Normalized DV Value')
    ax.legend(title='Surface', frameon=True, fancybox=True, shadow=True)
    
    fig.tight_layout()
    plt.savefig(output_path)
    plt.close(fig)
    print(f"Saved new Violin Plot Figure 5 to {output_path}")


# --- Main Execution ---
if __name__ == "__main__":
    print(f"\nGenerating publication-quality figures...")
    plot_figure_3_convergence(df_30, df_38, os.path.join(OUTPUT_DIR, 'Fig3_Final.png'))
    plot_figure_4_objective_space(df_30, df_38, os.path.join(OUTPUT_DIR, 'Fig4_Final.png'))
    plot_figure_5_violin_distribution(df_30, df_38, os.path.join(OUTPUT_DIR, 'Fig5_Final_Violin.png'))
